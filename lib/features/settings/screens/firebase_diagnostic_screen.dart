    ...
    try {
      ...
      try {
        ...
        if (e is FirebaseAuthException && e.code == 'user-not-found') {
          _authStatus =
              '✅ Firebase Auth is configured (user-not-found is expected)';
        } else if (e.toString().contains('CONFIGURATION_NOT_FOUND')) {
          _authStatus =
              '❌ Firebase Auth is not configured. Enable Email/Password sign-in method.';
        } else {
...
''';
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('Firebase credentials copied to clipboard')),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
